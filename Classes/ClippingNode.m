...
-(void) setClippingRegion:(CGRect)region
{
	...
	region.origin.x	+= position_.x;
	region.origin.y	+= position_.y;
...
-(void) visit
{
	// TODO set method for not clipping  boolean member

	if (clippingRegion.size.width != 0 && clippingRegion.size.height != 0 ) {
		glPushMatrix();
